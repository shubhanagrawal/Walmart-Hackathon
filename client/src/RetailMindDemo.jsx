...
const RetailMindDemo = () => {
  ...
  const handleNext = async () => {
    if (step === 5) {
      try {
        console.log('Submitting persona:', userPersona); // Debug log
        const response = await axios.post('http://localhost:5000/api/recommendations', {
          jobType: userPersona.job,
          budgetRange: `1000-${userPersona.budget}`,
          hobbies: userPersona.interests.join(','),
          favoriteBrands: userPersona.job === 'professional' ? 'Nike,JBL' : '',
          dietaryChoices: userPersona.lifestyle,
        });
        ...
        const filteredProducts = response.data
          .filter(product => product.price <= userPersona.budget && (userPersona.interests.some(interest => product.tags.includes(interest)) || (currentEvent?.products.some(p => product.name.toLowerCase().includes(p.toLowerCase())) ? 1.5 : 1.0) > 1))
          .sort((a, b) => {
            const aScore = userPersona.interests.filter(i => a.tags.includes(i)).length + (a.brand === 'Nike' ? 1 : 0);
            const bScore = userPersona.interests.filter(i => b.tags.includes(i)).length + (b.brand === 'Nike' ? 1 : 0);
        ...
        setError('Failed to fetch recommendations. Check backend logs or ensure it’s running.');
      }
    } else {
      setStep(step + 1);
    }
